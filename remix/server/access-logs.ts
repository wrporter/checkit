...
morgan.token('transactionId', (req: any, res: any) => {
    return req.traceContext?.transactionId;
});
morgan.token('requestId', (req: any, res: any) => {
    return req.traceContext?.requestId;
});
morgan.token('parentRequestId', (req: any, res: any) => {
    return req.traceContext?.parentRequestId;
});
...
const accessLogMiddleware = morgan(format, {
    stream: accessLogStream,
});

export function configureAccessLogs(app: Express) {
    app.use(setResponseBodyMiddleware);
